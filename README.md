...
timing, defined as CSS custom properties and remapped under `.dark`. The same
file's `@theme` block is the Tailwind v4 configuration: type scale, card radius,
z-index layers, breakpoints and fonts. There is no `tailwind.config.ts`.
Components reference the tokens rather than literal values, so a palette
change is a change in one file.
